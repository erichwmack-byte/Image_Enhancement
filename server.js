...
app.post('/api/enhance', upload.array('images'), async (req, res) => {
  try {
    const n8nUrl = 'https://n8n-production-9092.up.railway.app/webhook/Batch_EnhancementOptionsWeb';
...
app.get('/api/status', async (req, res) => {
  try {
    const statusUrl = `https://n8n-production-9092.up.railway.app/webhook/check-status?jobId=${req.query.jobId}`;
